...
//				CDC_Transmit_FS( vision_transmit_buff, VISION_BUFLEN );
				if(CDC_Transmit_FS( vision_transmit_buff, VISION_BUFLEN )==USBD_OK)
				{
				...
				set_gimbal_angle(&hcan2,imu_t.yaw);
				set_gimbal_flag(&hcan2,motor.Gimbal_Pitch.Apid.now,imu_t.yaw);
				//板间通信HAL_Delay(
//				if(test_can<3)
//					test_can++;
//				else
